...
RSpec.describe 'delete plumebrs' do
  ...
  it 'can click delete button and destroy plumbers from plumber index' do 
    visit "/plumbers"
  ...
  it 'can click delete button and destroy plumbers from plumber show' do 
    visit "/plumbers/#{@plumber_2.id}"

    click_button "Delete Plumber #{@plumber_2.name}"

    expect(current_path).to eq("/plumbers")
    expect(page).to_not have_content(@plumber_2.name)
  end 
end